...
%将所有预测准确率结果中去除404数据、统计有效天数、统计平均acc
ValidDayNum = 0;
tacc = zeros(5,1);
...
ValidSUR.Averacc = tacc / ValidDayNum;

%统计汇总结果:总频道数量、总推荐频道数量、总采纳频道数量、汇总acc、汇总召回率
UserTotalChanNum = 0;
for i = 1: size(datasetorder,2)   %累加每天的频道数
    UserTotalChanNum = UserTotalChanNum + size(datasetorder{1,i},1);
end
UserRecommChanNum = 0;
...
for i = 1: size(prediction,2)   
    if size(prediction{1,i},2)~= 0
        t = sum(prediction{1,i},2);
        trecall = trecall  + t/size(datasetorder{1,i},1);%累加每天的召回率
        UserAcceptedChanNum = UserAcceptedChanNum  + t;%累加每天的推荐top推荐准确次数
    end
end
...
ValidSUR.Averrecall = trecall / ValidDayNum;
